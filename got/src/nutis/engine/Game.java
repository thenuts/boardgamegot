...
public class Game {
  ...
  private Map<Integer, House> ironThrone = new HashMap<Integer, House>();
  private Map<Integer, House> fiefdoms = new HashMap<Integer, House>();
  ...
  public Game(GameRecord gameRecord) {
    ...
    for (HouseRecord houseRecord : gameRecord.getHouses()) {
      House house = House.fromRecord(houseRecord, map);
      houses.add(house);
      ironThrone.put(house.getIronThrone(), house);
      fiefdoms.put(house.getFiefdoms(), house);
      if(gameRecord.getHouse()!= null && gameRecord.getHouse()==houseRecord.getHouse()){
        this.house = house;
      }
    }
  }
  ...
  public GameRecord getRecord() {
    ...
    if (house != null) {
      result.setHouse(house.getType().getId());
    } else {
      result.setHouse(0); // TODO considerar nulo
    }
  ...
  public House getIronThrone(int i) {
    return ironThrone.get(i);
  }
